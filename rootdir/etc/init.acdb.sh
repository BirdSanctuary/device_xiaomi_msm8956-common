...
if [ -f "/system/etc/aanc_tuning_mixer.txt" ]
then
	print_log 5 $LOG_TAG "aanc audio configs has been copied already, skip..."
else
	cp -p /vendor/etc/aanc_tuning_*.txt /system/etc/  
	if [ $? -ne 0 ]
	then
		print_log 3 $LOG_TAG "Failed to copy aanc audio configs to /system/etc!"
	fi
fi
